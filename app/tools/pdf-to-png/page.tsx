...
import { ImageDown, Download, Archive, RotateCcw } from 'lucide-react';
...
export default function PdfToPngPage() {
  ...
  const handleFileSelected = async (files: File[]) => {
    ...
    try {
      setProgress(50);
      const pages = await renderPdfPages(selected, 30, 2.0); // 2x scale for sharp PNG
      setProgress(100);
      ...
      setError('Could not render PDF pages to PNG. Verify document format.');
    } finally {
  ...
  const handleDownloadSingle = (page: RenderedPage) => {
    ...
    a.href = page.dataUrl;
    a.download = `${file?.name.replace(/\.pdf$/i, '') || 'page'}_page_${page.pageNumber}.png`;
  ...
  const handleDownloadZip = async () => {
    ...
    renderedPages.forEach((p) => {
      const base64Data = p.dataUrl.replace(/^data:image\/(jpeg|png);base64,/, '');
      zip.file(`${baseName}_page_${p.pageNumber}.png`, base64Data, { base64: true });
    });
    ...
    const a = document.createElement('a');
    a.href = url;
    a.download = `${baseName}_png_images.zip`;
    document.body.appendChild(a);
    a.click();
    document.body.removeChild(a);
  ...
  return (
    <ToolLayout
      ...
      description="Extract PDF pages as crisp, lossless PNG pictures with sharp text and transparent background capabilities."
    ...
    >
      <div className="space-y-6">
        ...
        {isProcessing ? (
          <ProgressBar progress={progress} statusText="Rendering crisp PNG pages (2x resolution)..." />
            ...
            <div className="p-6 rounded-3xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-sm flex flex-col sm:flex-row items-center justify-between gap-4">
              <div>
                <h3 className="text-base font-bold text-slate-900 dark:text-white">
                  {renderedPages.length} High-Res PNG Pages Ready
                </h3>
                <p className="text-xs text-slate-400">Exported at 2.0x retina scale</p>
              </div>
          ...
          <FileUploader
            ...
            title="Drop a PDF file here to convert to PNG"
            subtitle="Select 1 PDF file to export as lossless images"
          />
        )}
      </div>
    </ToolLayout>
